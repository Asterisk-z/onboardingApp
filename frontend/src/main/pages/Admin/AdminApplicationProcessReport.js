...
import { loadApplicationProcessTimestamp } from "redux/stores/membership/applicationProcessStore"
import Content from "layout/content/Content";
import Head from "layout/head/Head";
import { loadAllActiveCategories } from 'redux/stores/memberCategory/category'
import AdminApplicationProcessReportTable from './Tables/AdminApplicationProcessReportTable'
...
const AdminApplicationReport = ({ drawer }) => {
    ...
    const process_report = useSelector((state) => state?.applicationProcess?.process_report) || null;
    const active_categories = useSelector((state) => state?.category?.list) || null;
    useEffect(() => {
        dispatch(loadAllActiveCategories());
        dispatch(loadApplicationProcessTimestamp());
    }, [dispatch, parentState]);


    const $process_report = process_report ? JSON.parse(process_report) : null;
    const $active_categories = active_categories ? JSON.parse(active_categories) : null;


    let filteredApplication = $process_report?.report;
                    ...
                    <Card className="card-bordered card-preview">
                        {false && <>
                            <Content>
                                ...
                                <Block size="xl">
                                    <BlockHead>
                                        <BlockHeadContent>
                                            <BlockTitle tag="h4">Application Process Report Filter</BlockTitle>
                                        </BlockHeadContent>
                                    </BlockHead>
                        ...
                        <Content>
                            ...
                            <Block size="xl">
                                ...
                                <PreviewCard>
                                    {$process_report && <AdminApplicationProcessReportTable updateParent={updateParentState} data={tableData || $process_report} className='overflow-auto' expandableRows pagination actions />}
                                </PreviewCard>
                            </Block>
