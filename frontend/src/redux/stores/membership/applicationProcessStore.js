...
export const loadApplicationProcessTimestamp = createAsyncThunk(
  "applicationProcess/loadApplicationProcessTimestamp",
  async () => {
    try {
      const { data } = await axios.get(`report/application/process`);
      return successHandler(data);
    } catch (error) {
      return errorHandler(error);
    }
...
const applicationProcess = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    // ====== builders for loadApplicationProcessTimestamp ======

    builder.addCase(loadApplicationProcessTimestamp.pending, (state) => {
      state.loading = true;
    });

    builder.addCase(loadApplicationProcessTimestamp.fulfilled, (state, action) => {
      state.loading = false;
      // state.list = action.payload?.data?.data?.categories;

      state.process_report = JSON.stringify(action.payload?.data?.data);

    });

    builder.addCase(loadApplicationProcessTimestamp.rejected, (state, action) => {
      state.loading = false;
      state.error = action.payload.message;
    });
