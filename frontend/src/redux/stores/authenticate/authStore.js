...
export const stakeHolderAccess = createAsyncThunk(
    "authenticate/stakeHolderAccess",
    async (values) => {
        try {
            const { data } = await axios({
                method: "post",
            ...
            if (data) {
                ...
                localStorage.setItem("logger", btoa(JSON.stringify(data.data.user)));
                localStorage.removeItem('reset-password-email')
                window.location.href = `${process.env.PUBLIC_URL}/admin-representatives-report`
            }
            console.log(data)
            return successHandler(data, "Access Granted");
        } catch (error) {
            if (error.response.data.statusCode == '666') {
                localStorage.setItem("reset-password-email", values.email);
            }
...
const authStore = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        // ====== builders for stakeHolderAccess ======

        builder.addCase(stakeHolderAccess.pending, (state) => {
            state.loading = true;
        });

        builder.addCase(stakeHolderAccess.fulfilled, (state, action) => {
            state.loading = false;
        ...
        builder.addCase(stakeHolderAccess.rejected, (state, action) => {
            state.loading = false;
            state.error = action.payload.message;
        });
